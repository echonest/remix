...
import lame, mad, numpy, eyeD3, tempfile, urllib
...
class Jingler:
    def __init__(self, audio, loudness, endOfFadeIn, beats, sections):
        ...
        if(audio.startswith("http")):
            ...
            foo, fileToWrite = tempfile.mkstemp(".mp3")
            urllib.urlretrieve(audio, fileToWrite)
            mf = mad.MadFile(fileToWrite)
        else:
...
def main( inputFilename, outputFilename ) :
    ...
    if(inputFilename.startswith("http")):
        track = audio.AudioAnalysis(inputFilename, audio.PARSERS)
    else:
