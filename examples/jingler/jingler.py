...
# constants
SLEIGH_OFFSET = -0.2 # seconds
SIGNAL_OFFSET = -1.0 # seconds
SANTA_OFFSET = -6.25 # seconds
MIN_BEAT_DURATION = 0.3 # seconds
...
class Jingler:
    def __init__(self, audio, loudness, endOfFadeIn, beats, sections):
        ...
        print "Loaded/Normalized audio in %.3f secs" % (time.time() - t1)
    ...
    def run(self, out):
        ...
        print "Sequenced and mixed in %.3f secs" % (time.time() - t1)
        # save
        t1 = time.time()
        print "Encoding mp3..."
        self.encode(out)
    ...
    def sequence(self, chops):
        # divide the duration by two if it sounds too fast
        ...
        for i in range(0,len(self.beats),stride):
            ...
            if beat.start+SLEIGH_OFFSET + 5 < self.duration:
                self.mix(beat.start+SLEIGH_OFFSET, seg=sample, volume=vol)
...
def main( inputFilename, outputFilename ) :
    ...
    if len(beats) < 5: 
        print 'not enough beats in this songs...'
        sys.exit(-2)
    # Get sections.
